...
The full flowchart (including the planned negative-sentiment Gmail alert branch) is in [`docs/presentation.pdf`](docs/presentation.pdf).
...
├── .gitignore
├── blueprint/
│   └── make-blueprint.json        ← Importable Make scenario (sanitized)
├── prompts/
│   └── gemini-sentiment-prompt.md ← The prompt + output contract
├── form/
│   └── customer-feedback-form.md  ← Form fields and structure
├── sheets/
...
1. **Clone this repo** and open `blueprint/make-blueprint.json`.
2. **Create the Google Form** described in [`form/customer-feedback-form.md`](form/customer-feedback-form.md). Note its Form ID.
3. **Link a responses sheet** and add the columns from [`sheets/google-sheets-schema.md`](sheets/google-sheets-schema.md).
...
The Gemini call uses a tightly-scoped prompt with a fixed output contract — see [`prompts/gemini-sentiment-prompt.md`](prompts/gemini-sentiment-prompt.md). The model returns:
